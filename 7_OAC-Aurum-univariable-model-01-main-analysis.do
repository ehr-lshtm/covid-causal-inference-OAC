/*=========================================================================
DO FILE NAME:			7_OAC-Aurum-univariable-model-01-main-analysis
...
log using "${pathLogs}/7_OAC-Aurum-univariable-model-01-main-analysis", text replace
...
*remove those who initiated OAC when pandemic started
...
save "$pathOutOAC/aurum_`doac'_`outcome'_analysis1", replace

stset stime_`outcome', id(groupid) fail(`outcome') enter(indexdate) origin(indexdate)	

cap file close tablecontent
file open tablecontent using "$pathResults/OAC/crude_`doac'_`outcome'_analysis1.txt", write text replace
...
save "$pathOutOAC/aurum_`doac'_`outcome'_analysis2", replace

stset stime_`outcome', id(groupid) fail(`outcome') enter(indexdate) origin(indexdate)	

cap file close tablecontent
file open tablecontent using "$pathResults/OAC/crude_`doac'_`outcome'_analysis2.txt", write text replace
...
save "$pathOutOAC/aurum_`doac'_`outcome'_analysis3", replace

stset stime_`outcome', id(groupid) fail(`outcome') enter(indexdate) origin(indexdate)	

cap file close tablecontent
file open tablecontent using "$pathResults/OAC/crude_`doac'_`outcome'_analysis3.txt", write text replace
...
*remove those who initiated OAC when pandemic restrictions ended
gen pandemic_end = mdy(04,17,2022)
...
cap file close tablecontent
file open tablecontent using "$pathResults/OAC/crude_`doac'_`outcome'_analysis4.txt", write text replace
