/*=========================================================================
Program for running the PS model for doac vs warfarin study for time-varying
need to specify global option for
$analysis
$interactionavailable (only for analysis 3)

*trimming regardless of period
*=============================================================================*/
gen initiate_period = 0 if indexdate < mdy(03,16,2020)
replace initiate_period = 1 if initiate_period ==. 

logistic exposure $psvarlist if initiate_period == 0, base
predict pscore0 if initiate_period == 0, pr
gen pscore = pscore0 if initiate_period == 0
drop pscore0

logistic exposure $psvarlist if initiate_period == 1, base
predict pscore1 if initiate_period == 1, pr
replace pscore = pscore1 if initiate_period == 1
drop pscore1
...
***********************************************************************/
gen wts = ipw_f
run "$pathPrograms/Weighted_STDs.do" 

cap file close tablecontent
file open tablecontent using "$stddiff", write text replace

file write tablecontent ("Table 1: Standardised differences after weighting - ATE") _n
file write tablecontent ("Variable") _tab ("SD") _n

foreach all_var in $psvarlist {

    local all_var: subinstr local all_var "i." ""
    local lab: variable label `all_var'
    file write tablecontent ("`lab'") _tab
	
	stddiff2 `all_var', by(exposure)
...
**********************************************************************/
stset stime_$outcome [pweight = ipw], id(groupid) fail($outcome) enter(time_enter_2) origin(indexdate)	
