...
* Plot PS graph by exposure (before weighting)
graph twoway kdensity pscore if exposure == 0 || ///
			 kdensity pscore if exposure == 1 , ///
			 graphregion(fcolor(white)) ///
			 legend(size(small) label(1 "`lab0'") label (2 "`lab1'") region(lwidth(none)) order(2 1))

graph export "$graphbeforeps", as(svg) replace
			 
* Plot PS graph by exposure (after weighting)
graph twoway kdensity pscore if exposure == 0 [fw = ipw_f] || ///
			 kdensity pscore if exposure == 1 [fw = ipw_f], ///
			 graphregion(fcolor(white)) ///
			 legend(size(small) label(1 "`lab0'") label (2 "`lab1'") region(lwidth(none)) order(2 1))

graph export "$graphafterps", as(svg) replace
